...
__global__ void d_reciprocal_grad(double *out_grad, const double *ret_grad, 
  ...
  const int step = blockDim.x * gridDim.x;
  for(int i = idx; i < size; i += step) {
    out_grad[i] = -ret_grad[i] / ((input[i] + epsilon) * (input[i] + epsilon));
  }
}
...
__global__ void d_log_grad(double *out_grad, const double *ret_grad, 
  ...
  double epsilon = 1e-7;
  for(int i = idx; i < size; i += step) {
    out_grad[i] = ret_grad[i] / (input[i] + epsilon);
  }
}
  ...
  if (device == "cpu") {
    ...
    if (requires_grad) {
      ...
      ret.backward_fn = [self, ret, epsilon]() mutable {
        if (self->requires_grad) {
          for (size_t i = 0; i < self->data.size(); ++i) {
            self->grad->at(i) += -ret.grad->at(i) / ((self->data[i] + epsilon) * (self->data[i] + epsilon));
          }
        }
      };
    }
  ...
  if (device == "cpu") {
    ...
    if (requires_grad) {
      ...
      ret.backward_fn = [self, ret, epsilon]() mutable {
        if (self->requires_grad) {
          for (size_t i = 0; i < self->data.size(); ++i) {
            self->grad->at(i) += ret.grad->at(i) / (self->data[i] + epsilon);
          }
        }
      };
    }
